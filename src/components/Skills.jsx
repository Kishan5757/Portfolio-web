...
export default function Skills() {
  ...
  const categoryTabs = [
    ...
    { id: 'aiml', name: 'ML & Data', icon: Brain },
    { id: 'tools', name: 'Tools & Tech', icon: Wrench },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <p className="mt-3 text-base text-slate-700 dark:text-slate-300 font-medium">
            Core programming languages, full-stack frameworks, machine learning libraries, and development tools I build with.
          </p>
        </div>
              ...
              <div key={cat.id} className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                  {cat.skills.map((skill, sIdx) => (
                    <div
                      key={sIdx}
                      className="glass-card glass-card-hover p-4 rounded-2xl border border-orange-300/50 dark:border-orange-500/20 shadow-xs group flex items-center gap-3 transition-all"
                    >
                      <div className="p-2.5 rounded-xl bg-orange-100/90 dark:bg-orange-950/40 text-orange-700 dark:text-orange-400 border border-orange-300/80 dark:border-orange-900/60 group-hover:scale-110 transition-transform shadow-xs shrink-0">
                        <DynamicIcon name={skill.icon} className="w-4 h-4" />
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="text-sm font-bold text-slate-950 dark:text-white group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors truncate">
                          {skill.name}
                        </div>
